        ...
        while idx < total_api_key:
            ...
            if (organic_results != ''):
                ...
                for item in organic_results:
                    cited_by = item['inline_links']['cited_by']['total'] if 'cited_by' in item['inline_links'].keys() else 0 
                    ...
                    if ('repository' not in link and link_pdf != ''):
                        if (cited_by in result_crawl.keys()):
                            result_crawl[cited_by].extend([link_pdf])
                        else:
                            result_crawl[cited_by] = [link_pdf]
                ...
                # Sorting berdasarkan sitasi tertinggi
                result_crawl = sorted(result_crawl.items(), reverse=True)
                
                try:
                    if (result_crawl):
                        found_keyword = False
                        for crawl in result_crawl:
                            for link in crawl[1]:
                                try:
                                    # Menghapus "temp.pdf"
